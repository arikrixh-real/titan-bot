...
try:
    from engines.roadmap_batch5_intelligence import run_roadmap_batch5_intelligence
    print("PHASES 51-53 ROADMAP BATCH 5 INTELLIGENCE CONNECTED")
except Exception:
...
try:
    from engines.roadmap_batch7_intelligence import run_roadmap_batch7_intelligence
    print("PHASES 57-59 ROADMAP BATCH 7 INTELLIGENCE CONNECTED")
except Exception:
    run_roadmap_batch7_intelligence = None

try:
    from engines.roadmap_batch8_intelligence import run_roadmap_batch8_intelligence
    print("PHASES 60-62 ROADMAP BATCH 8 INTELLIGENCE CONNECTED")
except Exception:
    run_roadmap_batch8_intelligence = None

try:
    from engines.roadmap_batch9_intelligence import run_roadmap_batch9_intelligence
    print("PHASES 63-65 ROADMAP BATCH 9 INTELLIGENCE CONNECTED")
except Exception:
...
def refresh_roadmap_batch5_safely(master_input=None, context=None, final_decisions=None):
    """
    Phases 51-53 advisory/research sidecars.

    Runs hierarchical brain organization, autonomous goal priority memory, and
    knowledge distillation in order so Phase 52 consumes Phase 51 and Phase 53
    consumes both. This writes only local memory/runtime/report artifacts and
    never changes live ranking, scanners, alert filtering, execution, broker
    state, Telegram, Supabase, dashboards, or live orders.
    """
    if run_roadmap_batch5_intelligence is None:
        print("[Phases51-53] Roadmap Batch 5 intelligence not connected.")
        return None

    try:
        result = run_roadmap_batch5_intelligence(
            master_input=master_input,
            context=context,
            final_decisions=final_decisions,
            write_files=True,
        )
        phase51 = result.get("phase51_hierarchical_brain_architecture", {}) if isinstance(result, dict) else {}
        phase52 = result.get("phase52_autonomous_goal_management", {}) if isinstance(result, dict) else {}
        phase53 = result.get("phase53_knowledge_distillation_engine", {}) if isinstance(result, dict) else {}
        print(
            "[Phases51-53] Batch 5 refreshed: "
            f"p51_run={phase51.get('run_count')} | "
            f"p52_uses51={phase52.get('phase51_consumed')} | "
            f"p53_uses51_52={phase53.get('phase51_consumed')}/{phase53.get('phase52_consumed')}"
        )
        return result
    except Exception as e:
        print(f"[Phases51-53 ERROR] Roadmap Batch 5 failed open: {e}")
        return {
            "error": str(e),
            "failed_open": True,
            "advisory_only": True,
...
def refresh_roadmap_batch7_safely(master_input=None, context=None, final_decisions=None):
    """
    Phases 57-59 advisory/research sidecars.

    Runs recursive self-reflection, long-term market memory, and institutional
    coordination in order so Phase 58 consumes Phase 57 and Phase 59 consumes
    both. This writes only local memory/runtime/report artifacts and never
    changes live ranking, scanners, alert filtering, execution, broker state,
    Telegram, Supabase, dashboards, or live orders.
    """
    if run_roadmap_batch7_intelligence is None:
        print("[Phases57-59] Roadmap Batch 7 intelligence not connected.")
        return None

    try:
        result = run_roadmap_batch7_intelligence(
            master_input=master_input,
            context=context,
            final_decisions=final_decisions,
            write_files=True,
        )
        phase57 = result.get("phase57_recursive_self_reflection_engine", {}) if isinstance(result, dict) else {}
        phase58 = result.get("phase58_long_term_market_memory", {}) if isinstance(result, dict) else {}
        phase59 = result.get("phase59_institutional_coordination_intelligence", {}) if isinstance(result, dict) else {}
        print(
            "[Phases57-59] Batch 7 refreshed: "
            f"p57_run={phase57.get('run_count')} | "
            f"p58_uses57={phase58.get('phase57_consumed')} | "
            f"p59_uses57_58={phase59.get('phase57_consumed')}/{phase59.get('phase58_consumed')} | "
            f"coordination={phase59.get('coordination_advisory')}"
        )
        return result
    except Exception as e:
        print(f"[Phases57-59 ERROR] Roadmap Batch 7 failed open: {e}")
        return {
            "error": str(e),
            "failed_open": True,
            "advisory_only": True,
...
def refresh_roadmap_batch8_safely(master_input=None, context=None, final_decisions=None):
    """
    Phases 60-62 advisory/research sidecars.

    Runs AGI transition, neuro-symbolic reasoning, and meta-cognition in order
    so Phase 61 consumes Phase 60 and Phase 62 consumes both. This writes only
    local memory/runtime/report artifacts and never changes live ranking,
    scanners, alert filtering, execution, broker state, Telegram, Supabase,
    dashboards, code, or live orders.
    """
    if run_roadmap_batch8_intelligence is None:
        print("[Phases60-62] Roadmap Batch 8 intelligence not connected.")
        return None

    try:
        result = run_roadmap_batch8_intelligence(
            master_input=master_input,
            context=context,
            final_decisions=final_decisions,
            write_files=True,
        )
        phase60 = result.get("phase60_agi_transition_layer", {}) if isinstance(result, dict) else {}
        phase61 = result.get("phase61_neuro_symbolic_reasoning_engine", {}) if isinstance(result, dict) else {}
        phase62 = result.get("phase62_meta_cognition_engine", {}) if isinstance(result, dict) else {}
        print(
            "[Phases60-62] Batch 8 refreshed: "
            f"p60_run={phase60.get('run_count')} | "
            f"p61_uses60={phase61.get('phase60_consumed')} | "
            f"p62_uses60_61={phase62.get('phase60_consumed')}/{phase62.get('phase61_consumed')} | "
            f"meta={phase62.get('meta_cognition_advisory')}"
        )
        return result
    except Exception as e:
        print(f"[Phases60-62 ERROR] Roadmap Batch 8 failed open: {e}")
        return {
            "error": str(e),
            "failed_open": True,
            "advisory_only": True,
...
def refresh_roadmap_batch9_safely(master_input=None, context=None, final_decisions=None):
    """
    Phases 63-65 advisory/research sidecars.

    Runs swarm intelligence, local federated-readiness, and advanced
    optimization in order so Phase 64 consumes Phase 63 and Phase 65 consumes
    both. This writes only local memory/runtime/report artifacts and never
    changes live ranking, scanners, alert filtering, execution, broker state,
    Telegram, Supabase, dashboards, code, or live orders.
    """
    if run_roadmap_batch9_intelligence is None:
        print("[Phases63-65] Roadmap Batch 9 intelligence not connected.")
        return None

    try:
        result = run_roadmap_batch9_intelligence(
            master_input=master_input,
            context=context,
            final_decisions=final_decisions,
            write_files=True,
        )
        phase63 = result.get("phase63_swarm_intelligence_architecture", {}) if isinstance(result, dict) else {}
        phase64 = result.get("phase64_federated_intelligence_system", {}) if isinstance(result, dict) else {}
        phase65 = result.get("phase65_advanced_optimization_framework", {}) if isinstance(result, dict) else {}
        print(
            "[Phases63-65] Batch 9 refreshed: "
            f"p63_run={phase63.get('run_count')} | "
            f"p64_uses63={phase64.get('phase63_consumed')} | "
            f"p65_uses63_64={phase65.get('phase63_consumed')}/{phase65.get('phase64_consumed')} | "
            f"optimization={phase65.get('optimization_readiness_score')}"
        )
        return result
    except Exception as e:
        print(f"[Phases63-65 ERROR] Roadmap Batch 9 failed open: {e}")
        return {
            "error": str(e),
            "failed_open": True,
            "advisory_only": True,
...
def _run_master_brain_unlocked(send_telegram=True, run_outcome_tracker=True, health_check=False):
    ...
    if not trade_window_open:
        ...
        roadmap_batch5_result = refresh_roadmap_batch5_safely(
            master_input={},
            context={},
            final_decisions={},
        )
        ...
        roadmap_batch7_result = refresh_roadmap_batch7_safely(
            master_input={},
            context={},
            final_decisions={},
        )
        roadmap_batch8_result = refresh_roadmap_batch8_safely(
            master_input={},
            context={},
            final_decisions={},
        )
        roadmap_batch9_result = refresh_roadmap_batch9_safely(
            master_input={},
            context={},
            final_decisions={},
        )
