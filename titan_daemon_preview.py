...
LOCK_NAME = "titan_daemon_preview"
...
def main():
    if not acquire_lock(LOCK_NAME):
        print("TITAN daemon preview already running. Exiting.")
        return
    ...
    try:
        while True:
            try:
                ...
                if now - last_printed_at >= PRINT_INTERVAL_SECONDS:
                    print(f"timestamp_ist: {dispatch_result['timestamp_ist']}")
                    print(f"mode: {dispatch_result['mode']}")
                    print(f"dispatch_preview: {dispatch_result['dispatch_preview']}")
                    last_printed_at = now
            except Exception as exc:
                log_runtime_error(
                    source="titan_daemon_preview_tick",
                    error=exc,
                    mode=latest_mode,
                )
        ...
        print("TITAN daemon preview stopped.")
    finally:
        write_daemon_health(
            mode=last_mode,
            ticks_completed=ticks_completed,
