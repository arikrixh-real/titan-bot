"""
TITAN Experience Maturity - Trap/Fakeout Memory Engine.

Persists advisory-only trap and fakeout outcome memory from existing journal and
experience records. It does not change trap detection, scanners, ranking,
Telegram, broker/API, or Supabase state.
"""

from __future__ import annotations

import csv
import json
from datetime import datetime
from pathlib import Path
from typing import Any, Dict, Iterable, List, Optional


PROJECT_ROOT = Path(__file__).resolve().parents[1]
MEMORY_PATH = PROJECT_ROOT / "data" / "memory" / "trap_fakeout_memory.json"
REPORT_PATH = PROJECT_ROOT / "reports" / "trap_memory_report.txt"

DEFAULT_RECORD_PATHS = [
    PROJECT_ROOT / "data" / "experience_vault" / "imported_trade_logs" / "historical_experience_import.jsonl",
    PROJECT_ROOT / "data" / "journals" / "trade_outcomes.jsonl",
    PROJECT_ROOT / "data" / "journals" / "trade_outcomes.csv",
    PROJECT_ROOT / "journal" / "trade_journal.csv",
]

STATE_VERSION = "1.0"
MAX_RECORDS = 300
MAX_BUCKETS = 50
MAX_EXAMPLES = 5
MAX_FILE_BYTES = 1_000_000

TRAP_TERMS = {
    "fake_breakout": ("fake breakout", "fakeout", "failed breakout", "breakout failed"),
    "bear_trap": ("bear trap", "failed breakdown", "breakdown reversal"),
    "bull_trap": ("bull trap", "failed upside", "upthrust"),
    "stop_run": ("stop run", "stop-run", "stop hunt", "stoploss hunt", "liquidity sweep", "sweep"),
    "general_trap": ("trap", "whipsaw", "failed follow through", "failed follow-through"),
}


def _now_text() -> str:
    return datetime.utcnow().isoformat() + "Z"


def _safe_float(value: Any, default: float = 0.0) -> float:
    try:
        if value is None or value == "":
            return default
        return float(value)
    except Exception:
        return default


def _safe_text(value: Any, default: str = "") -> str:
    try:
        if value is None:
            return default
        return str(value).strip()
    except Exception:
        return default


def _symbol(row: Dict[str, Any]) -> str:
    return _safe_text(row.get("symbol") or row.get("stock") or row.get("ticker") or "UNKNOWN").replace(".NS", "").upper()


def _normalise_outcome(row: Dict[str, Any]) -> Optional[str]:
    text = _safe_text(row.get("outcome") or row.get("result") or row.get("status") or row.get("trade_result")).upper()
    if text in {"WIN", "WON", "TP", "TARGET", "TARGET_HIT", "PROFIT", "SUCCESS"}:
        return "WIN"
    if text in {"LOSS", "LOST", "SL", "STOPLOSS", "STOP_LOSS", "SL_HIT", "FAILED"}:
        return "LOSS"
    return None


def _record_text(row: Dict[str, Any]) -> str:
    fields = [
        row.get("setup_type"),
        row.get("strategy"),
        row.get("reason"),
        row.get("setup_reason"),
        row.get("confirmations"),
        row.get("lesson_learned"),
        row.get("outcome_reason"),
        row.get("failure_cause"),
    ]
    return " ".join(_safe_text(item).lower() for item in fields if item is not None)


def detect_trap_patterns(row: Dict[str, Any]) -> List[str]:
    text = _record_text(row)
    patterns = []
    for pattern, terms in TRAP_TERMS.items():
        if any(term in text for term in terms):
            patterns.append(pattern)
    return patterns


def _new_bucket(pattern: str) -> Dict[str, Any]:
    return {
        "pattern": pattern,
        "samples": 0,
        "wins": 0,
        "losses": 0,
        "loss_rate": 0.0,
        "avg_score": 0.0,
        "examples": [],
    }


def _update_bucket(bucket: Dict[str, Any], row: Dict[str, Any], outcome: Optional[str]) -> None:
    samples = int(bucket.get("samples") or 0) + 1
    old_samples = samples - 1
    score = _safe_float(row.get("score") or row.get("final_score"), 0.0)

    bucket["samples"] = samples
    if outcome == "WIN":
        bucket["wins"] = int(bucket.get("wins") or 0) + 1
    elif outcome == "LOSS":
        bucket["losses"] = int(bucket.get("losses") or 0) + 1

    attempts = int(bucket.get("wins") or 0) + int(bucket.get("losses") or 0)
    bucket["loss_rate"] = round((int(bucket.get("losses") or 0) / attempts), 4) if attempts else 0.0
    bucket["avg_score"] = round(((_safe_float(bucket.get("avg_score")) * old_samples) + score) / samples, 4)

    examples = bucket.setdefault("examples", [])
    if len(examples) < MAX_EXAMPLES:
        examples.append(
            {
                "symbol": _symbol(row),
                "outcome": outcome or "UNKNOWN",
                "score": score,
                "source_type": _safe_text(row.get("source_type") or "EXPERIENCE_RECORD"),
            }
        )


def build_trap_memory(records: Iterable[Dict[str, Any]] | None = None) -> Dict[str, Any]:
    rows = [item for item in list(records or [])[-MAX_RECORDS:] if isinstance(item, dict)]
    pattern_buckets: Dict[str, Dict[str, Any]] = {}
    symbol_buckets: Dict[str, Dict[str, Any]] = {}
    matched_records = 0

    for row in rows:
        patterns = detect_trap_patterns(row)
        if not patterns:
            continue
        matched_records += 1
        outcome = _normalise_outcome(row)
        symbol = _symbol(row)
        for pattern in patterns:
            bucket = pattern_buckets.setdefault(pattern, _new_bucket(pattern))
            _update_bucket(bucket, row, outcome)

            symbol_key = f"{symbol}|{pattern}"
            symbol_bucket = symbol_buckets.setdefault(symbol_key, _new_bucket(pattern))
            symbol_bucket["symbol"] = symbol
            _update_bucket(symbol_bucket, row, outcome)

    ranked_symbols = sorted(
        symbol_buckets.items(),
        key=lambda item: (int(item[1].get("samples") or 0), _safe_float(item[1].get("loss_rate")), item[0]),
        reverse=True,
    )

    return {
        "version": STATE_VERSION,
        "generated_at": _now_text(),
        "source_type": "TRAP_FAKEOUT_MEMORY",
        "advisory_only": True,
        "affects_live_execution_directly": False,
        "record_count": len(rows),
        "matched_trap_records": matched_records,
        "pattern_buckets": dict(sorted(pattern_buckets.items())),
        "symbol_pattern_buckets": {key: value for key, value in ranked_symbols[:MAX_BUCKETS]},
        "memory_notes": [
            "Persists trap/fakeout outcome memory from existing records only.",
            "Does not change engines.trap_engine fake-breakout detection.",
            "Does not mutate scanner, ranking, alert, broker, or Supabase behavior.",
        ],
        "safety": {
            "broker_api_changes": False,
            "telegram_changes": False,
            "supabase_changes": False,
            "scanner_changes": False,
            "ranking_changes": False,
            "final_decision_changes": False,
            "live_order_allowed": False,
        },
        "rank_adjustment": 0.0,
        "recommended_live_weight": 0.0,
    }


def _read_jsonl(path: Path) -> List[Dict[str, Any]]:
    rows: List[Dict[str, Any]] = []
    for line in path.read_text(encoding="utf-8").splitlines()[-MAX_RECORDS:]:
        try:
            item = json.loads(line)
            if isinstance(item, dict):
                rows.append(item)
        except Exception:
            continue
    return rows


def _read_csv(path: Path) -> List[Dict[str, Any]]:
    with path.open("r", encoding="utf-8-sig", newline="") as handle:
        return [dict(row) for row in csv.DictReader(handle)][-MAX_RECORDS:]


def load_default_records(paths: Iterable[Path] | None = None) -> List[Dict[str, Any]]:
    rows: List[Dict[str, Any]] = []
    for path in paths or DEFAULT_RECORD_PATHS:
        try:
            if not path.exists() or path.stat().st_size > MAX_FILE_BYTES:
                continue
            if path.suffix.lower() == ".jsonl":
                rows.extend(_read_jsonl(path))
            elif path.suffix.lower() == ".csv":
                rows.extend(_read_csv(path))
        except Exception:
            continue
        if len(rows) >= MAX_RECORDS:
            break
    return rows[-MAX_RECORDS:]


def render_trap_memory_report(memory: Dict[str, Any]) -> str:
    pattern_buckets = memory.get("pattern_buckets") if isinstance(memory.get("pattern_buckets"), dict) else {}
    lines = [
        "TITAN Trap/Fakeout Memory Report",
        "=================================",
        "",
        "Safety",
        "- Advisory/research-only memory.",
        "- No scanner, ranking, final-decision, Telegram, broker/API, Supabase, or live-price mutation.",
        "",
        f"Updated: {memory.get('generated_at')}",
        f"Source Type: {memory.get('source_type')}",
        f"Advisory Only: {memory.get('advisory_only')}",
        f"Records: {memory.get('record_count', 0)}",
        f"Matched Trap Records: {memory.get('matched_trap_records', 0)}",
        "",
        "Pattern Buckets:",
    ]
    for pattern, bucket in pattern_buckets.items():
        if isinstance(bucket, dict):
            lines.append(
                f"- {pattern}: samples={bucket.get('samples', 0)}, wins={bucket.get('wins', 0)}, "
                f"losses={bucket.get('losses', 0)}, loss_rate={bucket.get('loss_rate', 0.0)}"
            )
    if not pattern_buckets:
        lines.append("- None observed")
    return "\n".join(lines) + "\n"


def refresh_trap_memory(records: Iterable[Dict[str, Any]] | None = None) -> Dict[str, Any]:
    source_records = list(records) if records is not None else load_default_records()
    memory = build_trap_memory(source_records)
    MEMORY_PATH.parent.mkdir(parents=True, exist_ok=True)
    MEMORY_PATH.write_text(json.dumps(memory, indent=2, sort_keys=True), encoding="utf-8")
    REPORT_PATH.parent.mkdir(parents=True, exist_ok=True)
    REPORT_PATH.write_text(render_trap_memory_report(memory), encoding="utf-8")
    return memory


if __name__ == "__main__":
    result = refresh_trap_memory()
    print("TITAN trap/fakeout memory refreshed")
    print("Matched:", result.get("matched_trap_records"))
    print("Memory:", MEMORY_PATH)
